...
public class QuoridorController {
	 ...
	 * 
	 * @param quoridor
	 *            - Quoridor in which to start new game
	 ...
	 * 
	 * @param No
	 *            parameters - The current player's clock will start counting down
	 * @throws Exception
	 * @author Tristan Bouchard
	 */
	 ...
	 *
	 * @param min
	 *            - Integer sets the number of minutes
	 * @param sec
	 *            - Integer sets the number of seconds
	 * @throws Exception
	 *             (UnsupportedOperationException) todo for later
	 * @author Nayem Alam
	 */
	 ...
	 *
	 * @param username
	 *            - String username exists within the list of users
	 * @throws Exception
	 *             (UnsupportedOperationException) todo for later
	 * @author Nayem Alam
	 */
	 ...
	 *
	 * @param username
	 *            - String username is new and does not exist in list of users yet
	 * @throws Exception
	 *             (UnsupportedOperationException) todo for later
	 * @author Nayem Alam
	 */
	 ...
	 * 
	 * @param row
	 *            , the row of the position of the pawn
	 * @param col
	 *            , the column of the pawn
	 ...
	 * 
	 * @param row
	 *            , is the row of the target tile of the WallMove associated to the
	 *            wall
	 * @param col
	 *            , the column of the target tile of the wall move associated to the
	 *            wall
	 * @param Walldir
	 *            , the direction of the WallMove associated with the wall
	 * @param wall
	 *            , the wall currently checked for validity
	 * @param id
	 *            , the id of the wall currently checked for validity
	...
	public static boolean initiatePosValidation(int row, int col, String Walldir, @Nullable Wall wall, int id) {
		...
		if (row >= 1 && row <= 8 && col >= 1 && col <= 8) {
			...
			int col1;
			int row1;
			for (int i = 0; i <= q.getCurrentGame().getCurrentPosition().numberOfBlackWallsOnBoard()
					+ q.getCurrentGame().getCurrentPosition().numberOfWhiteWallsOnBoard() - 1; i++) {
				if (id == i) {
					continue;
				}
				if (i > 9) {
	 ...
	 * 
	 * @param row
	 *            , the row of the tile of the WallMove associated with to the wall
	 * @param dir
	 *            , direction of the wall
	 * @param col
	 *            , column of the tile of the WallMove associated to the wall
	 ...
	 * 
	 * @param row
	 *            , row of the tile of the WallMove associated with the wall
	 * @param col
	 *            , column of the tile of the WallMove associated with the wall
	 * @param dir
	 *            , direction of the wall
	 * @param wall
	 *            , the wall which the position is checked
	 ...
	 * 
	 * @param row
	 *            row of the target tile for the move
	 * @param col
	 *            column of the target tile for the move
	 * @param dir
	 *            direction of the move
	 * @param aWall
	 *            wall to be moved
	 * @param player
	 *            player to whom the wall belongs
	 */
	...
	public static GamePosition loadSavedPosition(String filename) throws IOException {
		...
		// BIG PROBLEM NO? ALL THIS SHIT HAPPENS INSIDE LOOP WILL IT'S CHANGES HAPPEN
		// OUTSIDE OF LOOP
		...
		if (blackPawnValid = true) {
			...
			PlayerPosition blackPosition = new PlayerPosition(
					QuoridorApplication.getQuoridor().getCurrentGame().getBlackPlayer(), blackPawnTile);
		...
		for (int i = 2; i < whitePositionsFinal.length; i++) {
			...
			if (whiteWallOrientation.equals(Direction.Vertical)) {
				wallOrientation = "vertical";
			} else {
				wallOrientation = "horizontal";
			}
			whiteWallsValid = initiatePosValidation(whiteWallRow, whiteWallColumn, wallOrientation, whiteWall,
					whiteWall.getId());
			if (whiteWallsValid = true) {
				// whiteWallsOnBoard.add(whiteWall);
				// WallMove whiteWallMove = new WallMove(i-1, 1, getWhitePlayer(),
				// whiteWallTile, QuoridorApplication.getQuoridor().getCurrentGame(),
				// whiteWallOrientation, whiteWall);
		...
		for (int i = 2; i < blackPositionsFinal.length; i++) {
			...
			String wallOrientation = "";
			if (blackWallOrientation.equals(Direction.Vertical)) {
				wallOrientation = "vertical";
			} else {
				wallOrientation = "horizontal";
			}
			blackWallsValid = initiatePosValidation(blackWallRow, blackWallColumn, wallOrientation, blackWall,
					blackWall.getId());
			if (blackWallsValid = true) {
				// blackWallsOnBoard.add(blackWall);
				// WallMove blackWallMove = new WallMove(i-1, 1, getBlackPlayer(),
				// blackWallTile, QuoridorApplication.getQuoridor().getCurrentGame(),
				// blackWallOrientation, blackWall);
